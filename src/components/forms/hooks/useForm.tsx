...
type TSetErrorFunc = (name: string, error: ErrorOption, options?: { shouldFocus: boolean } | undefined) => void
...
export default function useForm<T extends FieldValues>(params: HookParams<T> = {}): HookResponse<T> {
  ...
  const { handleSubmit, reset, formState, control, setValue, getValues, trigger, watch, setError, unregister, setFocus } =
...
export function useFormContext() {
  const { handleSubmit, formState, watch, reset, getValues, setValue, trigger, setError, unregister, setFocus, clearErrors } =
